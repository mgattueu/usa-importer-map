...
<style>
:root{{
  --bg:#0f172a;--panel:#1e293b;--bd:#334155;--bd2:#475569;
  --tx:#e2e8f0;--tx2:#f1f5f9;--txd:#cbd5e1;--txf:#64748b;--txm:#94a3b8;
  --hover:rgba(255,255,255,.07);--lbg:rgba(15,23,42,.85);
}}
[data-theme=light]{{
  --bg:#f1f5f9;--panel:#ffffff;--bd:#e2e8f0;--bd2:#cbd5e1;
  --tx:#1e293b;--tx2:#0f172a;--txd:#475569;--txf:#94a3b8;--txm:#64748b;
  --hover:rgba(0,0,0,.05);--lbg:rgba(248,250,252,.93);
}}
*{{margin:0;padding:0;box-sizing:border-box}}
body{{font-family:'Segoe UI',sans-serif;background:var(--bg);color:var(--tx);height:100vh;display:flex;flex-direction:column;transition:background .2s,color .2s}}
#hdr{{padding:10px 16px;background:var(--panel);display:flex;align-items:center;gap:12px;border-bottom:2px solid var(--bd);flex-shrink:0;flex-wrap:wrap;transition:background .2s}}
#hdr b{{font-size:1rem;color:var(--tx2)}}
#hdr small{{font-size:.71rem;color:var(--txf)}}
.leg{{display:flex;gap:12px;margin-left:auto;flex-wrap:wrap;align-items:center}}
.li{{display:flex;align-items:center;gap:5px;font-size:.74rem;color:var(--txd)}}
...
#bar{{padding:6px 14px;background:var(--panel);display:flex;gap:7px;flex-wrap:wrap;align-items:center;border-bottom:1px solid var(--bd);flex-shrink:0;transition:background .2s}}
...
.b-self{{background:#3b82f6;color:#fff}}
.b-lbl{{background:var(--bg);color:var(--txm);border:1px solid var(--bd)}}
.b-theme{{background:var(--bg);color:var(--txm);border:1px solid var(--bd);font-size:.82rem;padding:2px 8px;line-height:1.6}}
#search{{margin-left:auto;padding:4px 12px;border-radius:13px;border:1px solid var(--bd2);background:var(--bg);color:var(--tx);font-size:.74rem;width:175px;transition:background .2s,color .2s}}
...
.loc-pin{{display:flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:50%;font-size:.72rem;font-weight:700;color:#fff;border:2.5px solid rgba(255,255,255,.7);box-shadow:0 2px 10px rgba(0,0,0,.6)}}
.loc-primary{{background:#3b82f6}}
.loc-secondary{{background:#7c3aed}}

#panel{{position:absolute;top:10px;left:10px;z-index:1000;background:var(--panel);border:1px solid var(--bd);border-radius:10px;width:290px;max-height:calc(100vh - 130px);overflow-y:auto;box-shadow:0 6px 28px rgba(0,0,0,.4);display:none;transition:background .2s}}
#panel-inner{{padding:14px}}
.pn{{font-weight:700;font-size:.95rem;margin-bottom:5px;line-height:1.3;color:var(--tx2)}}
...
.pv{{font-size:.8rem;font-weight:600;color:var(--tx);margin-bottom:10px}}
.addr-label{{font-size:.64rem;text-transform:uppercase;letter-spacing:.5px;color:var(--txf);margin-bottom:5px}}
...
.addr-item:hover,.addr-item.active{{background:var(--hover);border-color:var(--bd)}}
...
.addr-text{{font-size:.76rem;color:var(--txd);line-height:1.38}}
.addr-geo{{font-size:.63rem;color:var(--txf);margin-top:2px}}
.addr-warn{{color:#f59e0b}}

.plabel{{background:var(--lbg)!important;border:1px solid var(--bd)!important;color:var(--tx)!important;font-size:.7rem!important;padding:2px 6px!important;border-radius:4px!important;box-shadow:0 1px 4px rgba(0,0,0,.4)!important;white-space:nowrap!important;pointer-events:none!important}}
.plabel::before{{display:none!important}}
.mlabel{{display:flex;gap:4px;align-items:center}}
.mteu{{color:var(--txm);font-size:.65rem}}
.ftip{{background:var(--panel)!important;border:1px solid #7c3aed!important;color:var(--tx)!important;font-size:.73rem!important;padding:3px 8px!important;border-radius:4px!important;box-shadow:0 2px 6px rgba(0,0,0,.4)!important;white-space:normal!important;max-width:220px}}
.ftip::before{{border-right-color:#7c3aed!important}}

#stats{{position:absolute;bottom:20px;right:10px;z-index:999;background:var(--panel);border:1px solid var(--bd);border-radius:8px;padding:8px 12px;font-size:.72rem;min-width:150px;transition:background .2s}}
#stats h4{{color:var(--txf);text-transform:uppercase;letter-spacing:.4px;margin-bottom:4px;font-size:.67rem}}
.sr{{display:flex;justify-content:space-between;gap:10px;margin:2px 0}}
.sl{{color:var(--txm)}}.sv{{font-weight:700;color:var(--tx2)}}
</style>
</head>
<body>
<div id="hdr">
...
const darkTile=L.tileLayer('https://{{s}}.basemaps.cartocdn.com/dark_all/{{z}}/{{x}}/{{y}}{{r}}.png',{{attribution:'&copy; OpenStreetMap &copy; CartoDB',maxZoom:19}});
const lightTile=L.tileLayer('https://{{s}}.basemaps.cartocdn.com/light_all/{{z}}/{{x}}/{{y}}{{r}}.png',{{attribution:'&copy; OpenStreetMap &copy; CartoDB',maxZoom:19}});
darkTile.addTo(map);
let isDark=true;

// ── Icons ──────────────────────────────────────────────────────────
// Volume → radius: small range 5–38px so differences are obvious at a glance
function volToRadius(vol){{
  return Math.max(5, Math.min(5 + Math.sqrt(vol) * 2.8, 38));
}}

function makeCircleSvg(r, fill, opacity, strokeDash, vol, textColor){{
  const s=r*2+6;
  const fs=Math.max(7, Math.min(13, r*0.75));
  const lbl=vol>0?`<text x="${{s/2}}" y="${{s/2+fs*.38}}" text-anchor="middle" font-size="${{fs}}" font-weight="700" fill="${{textColor||'white'}}" font-family="Segoe UI,sans-serif">${{vol}}</text>`:'';
  const dash=strokeDash?`stroke-dasharray="${{strokeDash}}"`:'';
  return [`<svg xmlns="http://www.w3.org/2000/svg" width="${{s}}" height="${{s}}"><circle cx="${{s/2}}" cy="${{s/2}}" r="${{r}}" fill="${{fill}}" fill-opacity="${{opacity}}" stroke="white" stroke-width="1.5" ${{dash}}/>${{lbl}}</svg>`, s];
}}
...
function makeClusterIcon(cat,vol,geo){{
  ...
  const c = cat==='SELF'?'#3b82f6' : cat==='COMPETITOR'?compColor(vol):'#22c55e';
  const r = cat==='SELF' ? 14 : volToRadius(vol);
  const [svg, s] = makeCircleSvg(r, c, 0.92, null, cat==='SELF'?0:vol);
  return L.divIcon({{html:svg, className:'', iconSize:[s,s], iconAnchor:[s/2,s/2]}});
}}

function makeStateIcon(vol){{
  const r=volToRadius(vol);
  const [svg,s]=makeCircleSvg(r,'#7c3aed',0.85,'4,2',vol);
  return L.divIcon({{html:svg,className:'',iconSize:[s,s],iconAnchor:[s/2,s/2]}});
}}

function makeSecondaryIcon(cat,vol){{
  const c=cat==='COMPETITOR'?compColor(vol):(COLORS[cat]||'#888');
  // Secondary pins: same color, smaller (half radius), dashed
  const r=Math.max(4, volToRadius(vol)*0.55);
  const [svg,s]=makeCircleSvg(r,c,0.55,'3,2',0);
  return L.divIcon({{html:svg,className:'',iconSize:[s,s],iconAnchor:[s/2,s/2]}});
}}
